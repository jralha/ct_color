...
ct = ct.reshape(1,height,width,3)
...
model.fit(x=ct,y=Y,batch_size=1,epochs=100,verbose=0)
print(model.evaluate(ct, Y, batch_size=1))
...
cur[:,:,0] = ct[0,:,:,0]
cur[:,:,1:] = output[0,:,:,:]
out = (255*lab2rgb(cur)).astype(int)
